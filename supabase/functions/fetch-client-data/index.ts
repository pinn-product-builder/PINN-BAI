...
interface FetchRequest {
  ...
  orderBy?: string;
  orderAsc?: boolean;
  /** Filtro de período: aplica gte/lte em `dateField`. Opt-in — só filtra quando
   *  `dateField` é informado (e a coluna existe na fonte). */
...
serve(async (req) => {
  ...
  try {
    ...
    const { orgId, tableName, columns, filters, limit = 1000, orderBy, orderAsc = true, dateRange, dateField }: FetchRequest = await req.json();
    ...
    // Auth real: JWT validado + pertencimento à org (service_role bypassa RLS abaixo,
    // então o gate de tenant é aqui).
    const auth = await requireOrgAccess(req, orgId, corsHeaders);
    if (!auth.ok) return auth.response;

    // RPC parametrizada por período: tableName "rpc:<nome>" → chama a função no DB
    // interno com (p_org, p_start, p_end). Agrega no servidor (period-aware, sem
    // limite de linhas). p_start/p_end null = sem filtro de período.
    if (tableName.startsWith("rpc:")) {
      const rpcName = tableName.slice(4);
      const rpcArgs: Record<string, unknown> = {
        p_org: orgId,
        p_start: dateRange?.start ?? null,
        p_end: dateRange?.end ?? null,
      };
      // Repassa os filtros do widget como p_filters (ex.: {field_name}) só quando há —
      // RPCs sem esse parâmetro (kpis/funil) recebem só os 3 args.
      if (filters && Object.keys(filters).length > 0) rpcArgs.p_filters = filters;
      const { data, error } = await internalSupabase.rpc(rpcName, rpcArgs);
      if (error) {
        console.error("RPC error:", rpcName, error);
        return new Response(
          JSON.stringify({ error: `RPC ${rpcName} falhou: ${error.message}` }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
      const rows = Array.isArray(data) ? data : (data ? [data] : []);
      return new Response(
        JSON.stringify({ success: true, data: rows, count: rows.length, tableName }),
    ...
    if (!integrations || integrations.length === 0) {
      ...
      return await queryInternalDb(internalSupabase, orgId, tableName, columns, filters, limit, orderBy, orderAsc, corsHeaders, dateRange, dateField);
    }
...
async function queryInternalDb(
  ...
  // Filtro de período (opt-in): só aplica quando o widget informa `dateField`.
  // Cards sobre views pré-agregadas (sem coluna de data) não mandam dateField → sem filtro.
