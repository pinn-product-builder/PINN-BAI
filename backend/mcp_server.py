...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            "Retorna o funil real do CRM da organização — pipelines, estágios e contagem "
            "de leads abertos/ganhos/perdidos em cada um. Funciona para qualquer provider "
            "(Kommo, Ploomes, etc.) porque lê de crm_leads/crm_stages/crm_pipelines, as "
            "tabelas unificadas populadas pelo sync. Use para 'como está o funil?' ou "
            "'onde estão os leads parados?'. Caso o Kitou (Hermes pipeline de disparos), "
            "ele aparece como qualquer outro funil aqui — não precisa tool dedicada."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "org_id": {"type": "string", "description": "ID do tenant"},
                "pipeline_name": {
                    "type": "string",
                    "description": "Filtrar por pipeline específico (ex: 'Hermes', 'Disparos', 'SQL > Vendas'). Opcional — sem filtro retorna todos.",
                },
                "include_stages": {"type": "boolean", "default": True, "description": "Se true, devolve breakdown por estágio dentro de cada pipeline."},
            },
            "required": ["org_id"],
        },
    },
...
async def handle_tool(name: str, args: dict) -> Any:
    ...
    if name == "get_funnel_metrics":
        org_id = args["org_id"]
        pipeline_filter = (args.get("pipeline_name") or "").strip().lower()
        include_stages = args.get("include_stages", True)

        # Lê das tabelas crm_* unificadas (Kommo + Ploomes mapeados).
        leads_res = await db.table("crm_leads").select(
            "pipeline_external_id,stage_external_id,lead_status,opportunity_value,value"
        ).eq("tenant_id", org_id).execute()
        leads = leads_res.data or []

        pipelines_res = await db.table("crm_pipelines").select(
            "external_id,name"
        ).eq("tenant_id", org_id).execute()
        pipelines = pipelines_res.data or []

        stages_res = await db.table("crm_stages").select(
            "external_id,name,pipeline_external_id,sort_order,stage_type"
        ).eq("tenant_id", org_id).execute()
        stages = stages_res.data or []

        pipeline_by_id = {str(p["external_id"]): p for p in pipelines}
        stage_by_id = {
            (str(s["pipeline_external_id"]), str(s["external_id"])): s for s in stages
        }

        if not leads:
            return {"message": "Nenhum lead encontrado no CRM desta organização.", "funnels": []}

        funnels: dict[str, dict[str, Any]] = {}
        for l in leads:
            pid = str(l.get("pipeline_external_id") or "")
            if not pid:
                continue
            p = pipeline_by_id.get(pid, {})
            pname = str(p.get("name") or pid)
            if pipeline_filter and pipeline_filter not in pname.lower():
                continue
            if pid not in funnels:
                funnels[pid] = {
                    "pipeline_id": pid,
                    "pipeline_name": pname,
                    "open": 0, "won": 0, "lost": 0, "total_value_open": 0.0,
                    "stages": {},
                }
            status = (l.get("lead_status") or "open").lower()
            funnels[pid][status] = funnels[pid].get(status, 0) + 1
            if status == "open":
                v = l.get("opportunity_value") or l.get("value") or 0
                try:
                    funnels[pid]["total_value_open"] += float(v)
                except (TypeError, ValueError):
                    pass
            if include_stages:
                sid = str(l.get("stage_external_id") or "")
                if sid:
                    s = stage_by_id.get((pid, sid), {})
                    sname = str(s.get("name") or sid)
                    funnels[pid]["stages"].setdefault(sname, {"open": 0, "won": 0, "lost": 0, "sort_order": s.get("sort_order")})
                    funnels[pid]["stages"][sname][status] = funnels[pid]["stages"][sname].get(status, 0) + 1

        # Converte stages dict em lista ordenada por sort_order
        for f in funnels.values():
            if include_stages:
                f["stages"] = sorted(
                    [{"name": k, **v} for k, v in f["stages"].items()],
                    key=lambda x: (x.get("sort_order") is None, x.get("sort_order") or 0),
                )
            else:
                f.pop("stages", None)

        result = sorted(funnels.values(), key=lambda f: f["open"], reverse=True)
        return {
            "funnels": result,
