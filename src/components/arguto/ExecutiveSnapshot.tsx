...
export default function ExecutiveSnapshot({ onOpenChurn }: ExecutiveSnapshotProps = {}) {
  ...
  const openMetric = (m: MetricExplanation) => setActiveMetric(m);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {/* Conversão por visita — gauge antes/depois */}
      ...
      {/* ─── Banner de alertas (4 cards menores em linha) ─── */}
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] text-muted-foreground">— ação requerida nas próximas 72h</span>
        </div>
          ...
          {SECONDARY_ALERTS.map((alert) => {
            ...
            return (
              <Card
                ...
                className="p-4 border bg-card relative overflow-hidden cursor-pointer transition-all hover:shadow-md hover:-translate-y-0.5 group"
                  ...
                  <div className="min-w-0 flex-1 space-y-1.5">
                    ...
                    {alert.key === 'risco_churn' && churnEnabled && orgId && onOpenChurn && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-[10px] font-semibold mt-2 px-2 py-1 rounded-md border transition-all hover:bg-muted/40"
...
function ClickableKpiCard({
  ...
  return (
    <Card
      onClick={onClick}
      className={`relative overflow-hidden p-5 bg-card cursor-pointer transition-all hover:shadow-md hover:-translate-y-0.5 group ${
        accent === 'primary'
