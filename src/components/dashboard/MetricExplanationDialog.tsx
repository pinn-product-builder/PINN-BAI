...
import { BookOpen, Calculator, Database, ListTree, Lightbulb, Target, Clock } from 'lucide-react';

/**
 * Estrutura única que descreve uma métrica do dashboard. Cards a partir desta
 * estrutura ficam clicáveis e abrem o dialog mostrando fórmula, fontes e
 * detalhamento numérico. Padrão herdado do Arguto/MetricExplanationDialog.
 */
...
export function MetricExplanationDialog({ open, onOpenChange, explanation }: Props) {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-5 pt-2">
          <Section icon={BookOpen} label="O que é">
            <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-line">{explanation.definition}</p>
          </Section>
                ...
                <table className="w-full text-sm">
                  <tbody>
                    {explanation.breakdown.map((row, i) => (
                      <tr
                        key={i}
                        className={`border-b border-border/30 last:border-b-0 ${row.emphasis ? 'bg-primary/[0.04]' : ''}`}
                      >
                        <td className="px-3 py-2 text-muted-foreground text-[12px]">{row.label}</td>
                        <td className={`px-3 py-2 text-right tabular-nums ${row.emphasis ? 'font-bold text-primary' : 'font-medium text-foreground'}`}>
                          {row.value}
                        </td>
                      </tr>
                    ))}
          ...
          {explanation.interpretation && (
            <Section icon={Lightbulb} label="Como ler este número">
              <p className="text-sm text-foreground/85 leading-relaxed italic whitespace-pre-line">
                {explanation.interpretation}
              </p>
            </Section>
          )}
          ...
          {explanation.action && (
            <Section icon={Target} label="Próxima ação sugerida pelo BAI">
              <div className="p-3 rounded-md bg-primary/[0.05] border border-primary/20">
                <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-line">{explanation.action}</p>
              </div>
            </Section>
          )}
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-border/30 text-[11px] text-muted-foreground">
            ...
            <span>{explanation.refresh ?? 'Atualizado em tempo real conforme sincronização do CRM.'}</span>
          </div>
        </div>
      </DialogContent>
    </Dialog>
