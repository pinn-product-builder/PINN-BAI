...
import { Database, FileSpreadsheet, FileUp, Globe, Check, AlertCircle, Loader2, ArrowRight, RefreshCw, Briefcase, Building } from 'lucide-react';
import { Link as RouterLink } from 'react-router-dom';
// Reúso da camada CRM Auditor pra integrar Kommo/Omie inline no wizard
// (form de credenciais + persistência + sync sob demanda + auto-build do dashboard).
import { ProviderSetupCard } from '@/modules/crm-auditor/components/ProviderSetupCard';
...
interface IntegrationStepProps {
  ...
  /** Org sendo provisionada — necessário pra conectar CRM/ERP (Kommo/Omie) inline. */
  orgId?: string;
}

const INTEGRATION_TYPES = [
  {
    type: 'kommo' as IntegrationType,
    name: 'Kommo CRM',
    description: 'Subdomain + access token',
    icon: Briefcase,
    color: 'text-primary',
  ...
  {
    type: 'omie' as IntegrationType,
    name: 'Omie ERP',
    description: 'app_key + app_secret',
    icon: Building,
    color: 'text-primary',
    bgColor: 'bg-primary/10',
    externalSetup: true,
...
const IntegrationStep = ({ integration, onUpdate, orgId }: IntegrationStepProps) => {
        ...
        return (
          <InlineCrmSetup
            ...
            onConnected={(providers) =>
              onUpdate({
                id: integration?.id ?? `crm-${selectedType}`,
                type: selectedType,
                name: INTEGRATION_TYPES.find((t) => t.type === selectedType)?.name ?? selectedType,
...
/**
 * Integração CRM/ERP (Kommo/Omie) INLINE no wizard — sem redirecionar.
 * Reusa a camada do CRM Auditor: form de credenciais + persistência em
 * crm_auditor_connections + sync sob demanda (ProviderSetupCard), detecção da
 * 1ª sync (useFirstSyncDetector) e auto-build do dashboard (AutoBuildDashboardDialog).
 * Resultado: cola credencial → puxa tudo do CRM → dashboard se monta, nesta tela.
 */
function InlineCrmSetup({
  orgId,
  provider,
  onConnected,
}: {
  ...
  const label = provider === 'kommo' ? 'Kommo CRM' : 'Omie ERP';
  const firstSync = useFirstSyncDetector(orgId);
  const [buildOpen, setBuildOpen] = useState(false);
  const [dismissed, setDismissed] = useState(false);

  const ready = Boolean(firstSync.data?.ready);
  const connectedProviders = firstSync.data?.connectedProviders ?? [];

  // Quando a 1ª sync termina: marca a integração como conectada no wizard
  // e abre o auto-build do dashboard automaticamente.
  useEffect(() => {
    if (ready) {
      onConnected(connectedProviders);
      if (!dismissed) setBuildOpen(true);
    }
  ...
  if (!orgId) {
    return (
      <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
        Crie ou selecione a organização antes de conectar o {label}.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm text-muted-foreground">
        Cole as credenciais do <strong>{label}</strong>. Ao salvar, o sync roda na hora e puxa
        tudo do CRM; em seguida o dashboard se autoconstrói com as métricas (funil, conversão,
        pipeline, motivos de perda…).
      </div>

      {/* Form de credenciais + persistência + sync (reuso do CRM Auditor) */}
      <ProviderSetupCard orgId={orgId} provider={provider} />

      {ready && (
        <div className="flex flex-wrap items-center gap-3 rounded-lg border border-green-500/40 bg-green-50 p-3 text-sm">
          <Check className="h-4 w-4 text-green-600" />
          <span className="text-foreground">
            Sync concluído ({connectedProviders.join(', ') || provider}). Dados no ar.
          </span>
          <Button size="sm" onClick={() => { setDismissed(false); setBuildOpen(true); }}>
            Montar dashboard
          </Button>
        </div>
      )}
      ...
      <div className="text-xs text-muted-foreground">
        Prefere a tela completa?{' '}
        <RouterLink to="/crm-auditor" className="text-primary hover:underline">
          Abrir Auditor CRM <ArrowRight className="inline h-3 w-3" />
        </RouterLink>{' '}
        — dá pra combinar com outra fonte (Supabase, Sheets, CSV, API) depois.
      </div>

      <AutoBuildDashboardDialog
        open={buildOpen}
        orgId={orgId}
        dashboardId={firstSync.data?.dashboardId ?? null}
        connectedProviders={connectedProviders}
        onClose={() => { setBuildOpen(false); setDismissed(true); }}
        onApplied={() => { setBuildOpen(false); setDismissed(true); }}
      />
    </div>
  );
}
