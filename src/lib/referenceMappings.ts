...
export const REFERENCE_MAPPINGS: ReferenceMapping[] = [
  ...
  // ── TAXAS DE CONVERSÃO ────────────────────────────────────────────────
  // Cada conversão tem ID canônico distinto (ver src/lib/conversionMetrics.ts).
  // O auto-mapeamento aponta cada ID para sua coluna específica para evitar
  // que widgets com label "Taxa de Conversão" puxem de fontes diferentes
  // resultando em valores conflitantes na mesma tela.
  {
    targetMetric: 'conv_lead_to_meeting_30d',
    viewPatterns: [
      { tablePattern: /.*/,  fieldPattern: /conv_lead_to_meeting|lead_to_meeting/i, aggregation: 'avg' },
    ],
    defaultTransformation: 'percentage',
  },
  {
    targetMetric: 'conv_lead_to_won_30d',
    viewPatterns: [
      { tablePattern: /.*/,  fieldPattern: /conv_lead_to_won|lead_to_won|win_rate|taxa_ganho/i, aggregation: 'avg' },
    ],
    defaultTransformation: 'percentage',
  },
  {
    targetMetric: 'conv_stage_avg_30d',
    viewPatterns: [
      { tablePattern: /funil|stage|conversion/i, fieldPattern: /stage_conv|conv_stage|stage_to_stage/i, aggregation: 'avg' },
    ],
    defaultTransformation: 'percentage',
  },
  {
    // Genérico — mantido por compat com dashboards antigos. Tooltip avisa que
    // é ambíguo e orienta a configurar uma das 3 conversões canônicas acima.
    targetMetric: 'conversion_rate',
    viewPatterns: [
      { tablePattern: /kpi|dashboard/i,  fieldPattern: /conversion_rate|taxa_conversao/i,      aggregation: 'avg' },
      { tablePattern: /.*/,              fieldPattern: /^conversion_rate$|^taxa_conv$/i,        aggregation: 'avg' },
    ],
    defaultTransformation: 'percentage',
  },
  {
