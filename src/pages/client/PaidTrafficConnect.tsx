...
const PLATFORMS: Record<PlatformSlug, {
  ...
  meta_ads: {
    ...
    docsLabel: 'Como obter o token de acesso e o ID da conta de anúncios',
    fields: [
      { key: 'access_token', label: 'Token de Acesso', type: 'password', required: true, helper: 'Token de longa duração com permissão ads_read.' },
      { key: 'ad_account_id', label: 'ID da Conta de Anúncios', placeholder: '123456789 (sem o prefixo act_)', required: true, helper: 'Encontre em Gerenciador de Anúncios → Configurações da conta.' },
    ],
  },
  google_ads: {
    name: 'Google Ads',
    ...
    docsLabel: 'Como obter o token de desenvolvedor, refresh token e ID do cliente',
    fields: [
      { key: 'developer_token', label: 'Token de Desenvolvedor', type: 'password', required: true, helper: 'Em Ferramentas e configurações → Centro de APIs.' },
      { key: 'customer_id', label: 'ID do Cliente (Customer ID)', placeholder: '123-456-7890', required: true, helper: 'ID da conta Google Ads (com ou sem hífens).' },
      { key: 'login_customer_id', label: 'ID da Conta Gestora (MCC)', placeholder: 'opcional', helper: 'Preencha apenas se acessar via uma conta gerenciadora (MCC).' },
      { key: 'client_id', label: 'ID do Cliente OAuth', required: true },
      { key: 'client_secret', label: 'Chave Secreta OAuth', type: 'password', required: true },
